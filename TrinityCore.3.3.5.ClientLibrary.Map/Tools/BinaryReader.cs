...
public class BinaryReader
{
    ...
    public static BinaryReader? FromFile(string file, int skip = 0, int take = -1)
    {
        try
        {
            ...
            if (take == -1 || buffer.Length - skip < take) return new BinaryReader(buffer.Skip(skip).ToArray());
    ...
    public float[] ReadFloats(int len)
    {
        ...
        for (int i = 0; i < floats.Length; i++) floats[i] = ReadFloat();
        return floats;
    }
    ...
    public ushort[] ReadUShorts(int len)
    {
        ...
        for (int i = 0; i < ushorts.Length; i++) ushorts[i] = ReadUShort();
        return ushorts;
    }
